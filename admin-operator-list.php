...
session_start();
    ...
    $operator = "operator";
    $getalloperator = $conn->prepare("select * from tbl_users where user_level = ? order by f_name");
    $getalloperator->execute(array($operator));
?>
<!DOCTYPE html>
<html lang="en">
<head>
    ...
    <div id="wrapper">
        ...
        <main id="page-content-wrapper" role="main" class="container-fluid dash">
            <ol class="breadcrumb">
                ...
                <li class="breadcrumb-item"><a href="#">List of Operators</a></li>
            </ol>
          <div class="card table-responsive">
              <table class="table table-bordered">
                ...
                <?php
                    ...
                    while ($alloperator = $getalloperator->fetch()) {
                        $operatorid = $alloperator['user_id'];
                        $fname= $alloperator['f_name'];
                        $lname = $alloperator['l_name'];
                        $email = $alloperator['email'];
                        $number = $alloperator['contact_no'];
                ?>
                    <tr>
                        ...
                        <td>
                            <?php
                                $getjeepno = $conn->prepare("select jeepney_no from tbl_jeepbyoperator where user_id = ?");
                                $getjeepno->execute(array($operatorid));
                                while ($jeepno = $getjeepno->fetch()) {
                                    echo $jeepno['jeepney_no']. "<br>";
                                }
                            ?>
                        </td>
                            ...
                            <div class="row">
                                <div class="col-xs-12">
                                    <p data-placement="top" data-toggle="tooltip" title="Edit">
                                        <a href="admin-edit-operator.php?id=<?php echo $operatorid; ?>">
                                ...
                                <div class="col-xs-12">
                                    <p data-placement="top" data-toggle="tooltip" title="Delete">
                                        <button class="btn btn-danger btn-xs modal-delete btn-block" data-id="<?php echo $operatorid; ?>" data-title="Delete" data-toggle="modal" data-target="#modal-delete" >
                                            Delete
...
<div class="modal fade mdal" id="modal-delete" role="dialog">
  <div class="modal-dialog modal-md">
      <div class="modal-content">
          ...
          <form action="admin-delete-operator.php" method="post">
            <div class="modal-body">
              <input type="hidden" id="SrcId" name="srcid">
              <h4 style="text-align: center;">Are you sure you want to delete this Operator?</h4>
            </div>
    ...
    <script>
        $(function() {
            ...
            <?php if (isset($_SESSION['operator_delete'])): ?>
              swal("Success!", "Operator successfully deleted", "success");
              <?php unset($_SESSION['operator_delete']) ?>
            <?php endif ?>
        });
    </script>
</body>
